...
public class UserInfoActivity extends BaseActivity {
    ...
    protected void getDataFormWeb() {
        ...
        Observable.create(new ObservableOnSubscribe<ActionResult<PersonBean>>() {
            @Override
            public void subscribe(ObservableEmitter<ActionResult<PersonBean>> e) throws Exception {
                ActionResult<PersonBean> nr = mainDao.getPersonById(param);
                e.onNext(nr);
            }
        }).subscribeOn(Schedulers.io()) // 指定 subscribe() 发生在 IO 线程
                .observeOn(AndroidSchedulers.mainThread()) // 指定 Subscriber 的回调发生在主线程
                .subscribe(new Consumer<ActionResult<PersonBean>>() {
                    @Override
                    public void accept(ActionResult<PersonBean> result) throws Exception {
                        progressDialog.dismiss();
                        if (result.getCode() != 0) {
                            Toast.makeText(TheApplication.getContext(), result.getMessage(), Toast.LENGTH_SHORT).show();
                            return;
                        } else {
                            PersonBean dataResult = result.getResult();
